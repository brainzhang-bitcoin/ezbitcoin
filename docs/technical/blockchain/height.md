...
The height of a [block](/technical/block/) indicates its **position in the [blockchain](/technical/blockchain/)**.
...
The [target](/technical/mining/target/) adjusts after every 2,016 blocks (roughly every 2 weeks).

This helps to keep a **10-minute interval between blocks** as [miners](/technical/mining/) join and leave the network over time.
...
The [block subsidy](/technical/mining/block-reward/#block-subsidy) halves after every 210,000 blocks (roughly every 4 years).
...
For example, the block subsidy started at **50 BTC**. Then at the block height of [210,000](/explorer/210000#blockchain) it halved to **25 BTC**, and at block height [420,000](/explorer/420000#blockchain) it halved to **12.5 BTC**, and so on. At block height 6,930,000 (and after 33 [halvings](/technical/mining/block-reward/#halving-table) in total) the subsidy will reach **zero**.

### Other

The *height* is used in a few other places in Bitcoin, primarily to do with the eligibility for [transactions](/technical/transaction/) to get mined:
...
The [locktime](/technical/transaction/locktime/) field can be used to **prevent a transaction from being mined until *after* a specific height**.
...
[Relative locktime](/technical/transaction/input/sequence/#relative-locktime) can be used to prevent a transaction from being mined until the [output](/technical/transaction/output/) it's spending has reached a certain *depth* in the blockchain.

For example, if you set a relative locktime of **100** blocks on a transaction [input](/technical/transaction/input/) spending the output of a transaction in block **500,000**, that transaction can only get mined into a block at a height of **500,101** or above.
...
Starting from block [227,836](/explorer/227836#blockchain), all [coinbase transactions](/technical/mining/coinbase-transaction/) **must contain the height** of the block they are going to be mined in.

This forces each coinbase transaction to have a unique [TXID](/technical/transaction/input/txid/), as before this it was possible for coinbase transactions in different blocks to have the [same TXID](/technical/transaction/input/txid/#duplicate).
...
During the [mining](/technical/mining/) process, it's possible that two blocks will get mined at the same time. Therefore, there can be two different blocks competing for the same height in the blockchain:
...
This is known as a [chain reorganization](/technical/blockchain/chain-reorganization/).
...
* **The [block hash](/technical/block/hash/) is the most reliable way to reference a block.** A block hash always references a specific block, whereas the height is more of a *descriptor* than a unique identifier.
* **The height gets more reliable the further a block makes it down the blockchain.** If a block makes it 3+ blocks deep in the blockchain, it's highly unlikely that it's going to be switched out due to a [chain reorganization](/technical/blockchain/chain-reorganization/).
...
If your node holds multiple blocks at the same height, this command will return the block hash for the block that is part of the current [longest chain](/technical/blockchain/longest-chain/). If there are multiple blocks at the tip of your chain, your node will consider the *first* block it receives as part of the current longest chain (but again, this is liable to change if there is a chain reorganization).
...
You're better off using the [block hash](/technical/block/hash/) to reliably reference a block though, as the blocks near the top of the blockchain can change due to [chain reorganizations](/technical/blockchain/chain-reorganization/).
