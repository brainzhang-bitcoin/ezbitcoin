...
This is when changes are made to the rules so that previously invalid [blocks](/technical/block/)/[transactions](/technical/transaction/) will now be considered valid in the new version of the software.

As a result, unless everyone upgrades to the new software, the [blockchain](/technical/blockchain/) will split in two different directions:
...
The old [nodes](/technical/networking/node/) cannot accept these new blocks, so **two parallel blockchains** will exist. This is known as a "hard fork" in the chain.
...
To give a simple example, let's say the block size limit is 1 MB (block capacity is now measured in [weight](/technical/block/#weight), but don't worry about that for now), and let's say we upgrade the software to accept blocks up to **10 MB**.
...
This hard fork will persist as long as the new chain (containing incompatible 10 MB blocks) remains as the [longest chain](/technical/blockchain/longest-chain/) on the network.
...
2. **Changes to transaction structure.**
   * e.g. Changing the [cryptography](/technical/cryptography/) used for [signatures](/technical/keys/signature/) on existing transactions.
3. **Changes to other consensus rules.**
   * e.g. Increasing the maximum supply of bitcoin.
   * e.g. Manually adjusting the [target](/technical/mining/target/) to make it easier to mine blocks.
...
You can make changes to the rules using what's known as a [soft fork](/technical/blockchain/soft-fork/). However, depending on the complexity of the upgrade, a hard fork would typically be the *simpler* option for making changes to the Bitcoin software.
...
This is not a user-friendly way to upgrade a peer-to-peer [network](/technical/networking/).
...
**A split between miners will also split the mining power between the two chains.** Each chain would therefore be *less secure*, as each chain will have less power to protect against a [51% attack](/technical/blockchain/51-attack/).
...
* **Fixing the `OP_CHECKMULTISIG` [Script](/technical/script/) opcode.** This opcode pops off one more item from the stack than it should (see [P2MS](/technical/script/p2ms/)).
* **Increasing the size of the [nonce](/technical/block/nonce/) field in the block header.** The nonce field is too small, which means miners have to resort to modifying the [coinbase transaction](/technical/mining/coinbase-transaction/) (see [ExtraNonce](/technical/block/nonce/#extranonce)) to continue [mining](/technical/mining/) the block after all the possible numbers in the nonce field have been exhausted.
* **Consistency with [byte order](/technical/general/byte-order/).** The [block hash](/technical/block/hash/) is displayed in [reverse byte order](/technical/general/byte-order/#reverse-byte-order) when searching for a block on an [explorer](/explorer/), but the block hashes used inside raw block data are in [natural byte order](/technical/general/byte-order/#natural-byte-order). It would be easier for developers if all block hashes were in *reverse byte order* internally.
...
* **Switching away from the SHA-256 [hash function](/technical/cryptography/hash-function/).** This would be required if a weakness is found that undermines the effort required to mine blocks.
* **Manually increasing the target value (i.e. reducing the [difficulty](/beginners/guide/difficulty/)).** This would be required if there was an irrecoverable loss of mining power on the network, which would result in painfully slow block intervals. Without a hard fork, it could take months before the next target adjustment and for transactions to be mined in a timely manner again.
...
Bitcoin uses a separate [chainstate database](https://github.com/in3rsha/bitcoin-chainstate-parser) to keep track of all the [UTXOs](/technical/transaction/utxo/). Versions prior to v0.8.0 used the [BerkeleyDB](https://www.oracle.com/database/technologies/related/berkeleydb.html) database.

Due to the way BerkeleyDB works, it imposed a limit on the number of [inputs](/technical/transaction/input/) that could be included inside a [block](/technical/block/). This created an unexpected and unintentional network consensus rule about what makes a valid block.
...
This was initially remedied by asking miners to downgrade to v0.7.0, which meant that the majority of the mining power was back on the old v0.7.0 chain. This led to the v0.7.0 chain becoming the longest chain once again, and all nodes on v0.8.0 [reorganized](/technical/blockchain/chain-reorganization/) back to this chain, temporarily solving the problem and forcing all nodes back on to the v0.7.0 chain.
...
In short, versions 0.15.0 to 0.16.2 would allow for a miner to double-spend the same UTXO if it was included as an [input](/technical/transaction/input/) multiple times in the same transaction. Only miners would be able to perform this attack, as nodes would only accept this transaction if it was already mined into a block (they would reject the transaction on its own otherwise).
...
A *hard fork* is when you make an upgrade to the Bitcoin software that will produce new [blocks](/technical/block/)/[transactions](/technical/transaction/) that are **incompatible with the old version of the software**.

To make a successful hard fork, you want everyone running Bitcoin (i.e. [nodes](/technical/networking/node/) and [miners](/technical/mining/)) to upgrade to the new version of the software. This is so everyone will receive these new blocks, and agree on the same upgraded version of the [blockchain](/technical/blockchain/).
...
There have yet to be any intentional hard-forking changes to Bitcoin. Instead, general upgrades to the software are preferred to be made using [soft forks](/technical/blockchain/soft-fork/).
