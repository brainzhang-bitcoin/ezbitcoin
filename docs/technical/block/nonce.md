...
The nonce is a spare field at the end of the [block header](/technical/block/#header) used for [mining](/technical/mining/).
...
Anyway, the easiest way to understand its purpose is to adjust the nonce in a block header to see how it affects the [block hash](/technical/block/hash/):
...
Miners increment the nonce value when mining so that they can get completely different [hash](/technical/cryptography/hash-function/) results for the block header of their [candidate block](/technical/mining/candidate-block/). They hope to stumble upon a "magic" nonce value that will produce a block hash that is below the current [target](/technical/mining/target/).
...
When a miner exhausts the nonce field the obvious next step is to adjust the [time](/technical/block/time/) field. This produces a slightly different block header, which allows the miner to increment through the nonce field again in an attempt to mine the same block of transactions.
...
The ExtraNonce is located inside [scriptSig](/technical/transaction/input/scriptsig/) of the [coinbase transaction](/technical/mining/coinbase-transaction/). Miners are free to put any data they like in the scriptSig, which means they can modify it to serve as an **unofficial field for an additional nonce value**.

This works because:

1. Changing the contents of the [scriptSig](/technical/transaction/input/scriptsig/) changes the transaction data.
2. Changing the transaction data for the coinbase transaction changes its [TXID](/technical/transaction/input/txid/).
3. Changing the TXID for the coinbase transaction changes the [merkle root](/technical/block/merkle-root/).
...
* The [version](/technical/block/version/) field can also be used as an extra nonce.
