...
它连接到[网络](/docs/technical/networking.md)上的其他节点，以分享有关[交易](/docs/technical/transaction.md)和[区块](/docs/technical/block.md)的信息。
...
当你第一次运行比特币时，它会连接到网络上的其他节点来下载一份完整的[区块链](/docs/technical/blockchain.md)副本。
...
注意：这是我本地节点的区块链大小。  
你的区块链大小会有所不同，具体取决于你的节点经历过多少次[链重组](/docs/technical/blockchain/chain-reorganization.md)以及你在磁盘上存储了多少[陈旧区块](/docs/technical/blockchain/chain-reorganization.md#stale-blocks)。

比特币非常像[种子下载 (torrent)](https://en.wikipedia.org/wiki/BitTorrent)，许多不同的计算机都在做种同一个文件（区块链）。

### 2. 验证并中继新交易和区块。

在下载了最新副本的区块链之后，节点就可以**开始接收最新的[交易](/docs/technical/transaction.md)和[区块](/docs/technical/block.md)**。
...
新交易会被添加到[内存池](/docs/technical/mining/memory-pool.md)。
...
新区块会被写入[区块链](/docs/technical/blockchain.md)。
...
:   推荐：2+ TB
    ...
    * 当前区块链大小：856.92 GB
    ...
    首先也是最重要的一点，你需要一个足够大的硬盘来存储[区块链](/docs/technical/blockchain.md)。
...
:   推荐：2+ GB
    ...
    * 当前内存池大小：2.57 vMB
    ...
    内存用于存储[内存池](/docs/technical/mining/memory-pool.md)中的最新交易，以及存储 [UTXO](/docs/technical/transaction/utxo.md) 以帮助加快新交易和区块的验证。
...
:   推荐：2+ TB/月
    ...
    * 传出：平均 25.20 GB/天
    ...
    节点会不断地与网络中的其他节点发送和接收数据（[交易](/docs/technical/transaction.md)和[区块](/docs/technical/block.md)），因此你需要足够的带宽来满足这一需求。
...
节点通过发送许多单独的[消息](/docs/technical/networking.md#messages)与其他节点通信。
...
**比特币网络是完全开放且任何人都可以访问的。** 所以只要你遵守连接和发送消息的规则，任何人都可以编写自己的软件来与节点通信。详情请参见[网络](/docs/technical/networking.md)。
...
运行自己的节点意味着你**不需要与第三方服务共享你的[交易](/docs/technical/transaction.md)**。

如果你不运行自己的节点，你需要使用运行节点的第三方网站或钱包来替你向[网络](/docs/technical/networking.md)发送交易。这些第三方服务可以跟踪你的请求以及你的 IP 地址，以构建你活动的全貌。

你可以想象，这对于隐私来说并不好。

但是，通过运行自己的节点，你可以直接通过自己的节点广播交易，因此它们在进入网络之前不再通过中间人。同样，你可以从自己的区块链中获取数据，而无需使用第三方的区块链浏览器网站。

Again，我到目前为止使用值得信赖的[钱包](/docs/beginners/wallets.md)或[区块链浏览器](/explorer/)从没遇到过问题，但重要的是要意识到如果这样做可能会存在隐私泄露的*潜在风险*。

### 3. 支持网络

运行你自己的节点以两种方式支持网络：

1. **区块链复制。** 比特币很难被消灭，因为交易的整个历史在世界各地被复制，所以向网络中添加另一个节点会让比特币*更具韧性*。例如，如果网络中的所有其他节点都爆炸了并丢失了其[区块链](/docs/technical/blockchain.md)副本，你将有效地维持整个系统，直到其他节点可以从你那里重新下载区块链。
2. **数据传输。** 比特币之所以起作用，是因为许多单独的节点协同工作以在网络中传播最新的[交易](/docs/technical/transaction.md)和[区块](/docs/technical/block.md)。所以通过运行节点，你是在向网络添加另一个中继。例如，如果有一大批节点下线，并且某些节点由于某种原因无法相互连接，你的节点可能最终会成为网络不同部分之间的关键纽带。
...
节点会将所有 [UTXO](/docs/technical/transaction/utxo.md) 保存在单独的数据库中，所以即使剪枝节点在进行中删除了较旧的区块，它也将始终具有 UTXO 的完整副本以供参考，从而使其能够验证新的交易和区块。
...
SPV 钱包只接收区块链的[区块头](/docs/technical/block.md#header)（它比完整的区块小得多），这使它们能够跟上最长链**看起来的样子**：
...
这被称为 [Merkle 证明](/docs/technical/block/merkle-root.md#merkle-proof)。
...
对于全节点来说，以这种方式向 SPV 钱包说谎需要付出很多努力，因为全节点需要故意[开采](/docs/technical/mining.md)一个无效区块。因此，SPV 钱包在全节点为说谎而付出的代价太高的假设下运行。

如果你想在不信任任何人的情况下*确保*你看到的所有交易都是有效的，你需要运行一个全节点。

### 矿工

矿工是致力于从[内存池](/docs/technical/mining/memory-pool.md)中提取交易并将其添加到[区块链](/docs/technical/blockchain.md)中的人。
...
然而，**矿工不需要执行节点的工作**。相反，矿工可以简单地连接到全节点以获取构建[候选区块](/docs/technical/mining/candidate-block.md)所需的信息，然后在完成后将区块发送回全节点。
...
比特币[网络](/docs/technical/networking.md)是完全开放的，因此如果你能弄清楚如何[连接](/docs/technical/networking.md#connecting)到其他节点并遵守网络的规则（即你可以发送并接收[交易](/docs/technical/transaction.md)和[区块](/docs/technical/block.md)），那么你将能够使用自己独有的软件来使用比特币。这非常酷。
