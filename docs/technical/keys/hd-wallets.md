...
所以基本上，HD Wallets 允许你使用单个 [seed](/docs/technical/keys/hd-wallets/mnemonic-seed.md) 生成数十亿个[私钥](/docs/technical/keys/private-key.md)。因此，只要你记住 seed，你就始终能够恢复相同的密钥和[地址](/docs/technical/keys/address.md)。

这使得它们比早期单独生成和存储私钥的[比特币钱包](/docs/beginners/wallets.md)更加用户友好。
...
当你创建 HD Wallets 时，你会获得一个包含 12 或 24 个单词的[助记词](/docs/technical/keys/hd-wallets/mnemonic-seed.md)。这是 *seed* 的来源，然后用于生成钱包中的所有密钥和地址。
...
你的 HD Wallets 中的密钥将根据你想使用的[地址](/docs/technical/keys/address.md)类型，使用以下[派生路径](/docs/technical/keys/hd-wallets/derivation-paths.md)之一生成：
...
在基础钱包中，每次你想接收比特币时，都会独立生成[私钥](/docs/technical/keys/private-key.md)和[公钥](/docs/technical/keys/public-key.md)对。
...
然而，在分层确定性钱包中，你可以使用单个 **[seed](/docs/technical/keys/hd-wallets/mnemonic-seed.md)** 来创建 **[主私钥](/docs/technical/keys/hd-wallets/extended-keys.md#master-extended-key)**，并可以使用它来生成数十亿个“子”私钥和公钥。
...
钱包中的每个[**子密钥**](/docs/technical/keys/hd-wallets/extended-keys.md#child-key-derivation)也可以**生成它自己的密钥**，这意味着你可以创建一个**树状结构**（或*层级*）来组织钱包中的密钥。
...
有关技术细节，请参阅[扩展密钥](/docs/technical/keys/hd-wallets/extended-keys.md)。
...
要创建 HD Wallets，首先要生成 64 个随机[字节](/docs/technical/general/bytes.md)。这就是我们的 **seed**。
...
链码只是额外的 32 字节，我们将它与私钥结合起来创建我们所说的[扩展密钥](/docs/technical/keys/hd-wallets/extended-keys.md)。
...
扩展密钥内嵌入的私钥可以像往常一样用于创建相对应的[公钥](/docs/technical/keys/public-key.md)：
...
仅 x 公钥用于 [Taproot](/docs/technical/upgrades/taproot.md) 输出。相对应的 y 坐标被假定为偶数。
...
所以本质上，新的私钥是通过将主扩展私钥与**索引**号一起进行[哈希](/docs/technical/cryptography/hash-function.md)生成的。
...
在哈希父扩展私钥后计算子私钥时有一个额外的数学步骤。这就是为什么你无法通过简单地将 (32字节私钥 | 4字节索引) 和 (32字节链码) 输入到 HMAC 中来获得正确结果。详情请参阅[扩展密钥](/docs/technical/keys/hd-wallets/extended-keys.md)。
...
现在，因为这次子密钥已根据**父**私钥和公钥进行了*调整*，[椭圆曲线](/docs/technical/cryptography/elliptic-curve.md)数学的魔力意味着**子**私钥和公钥将一一对应。
...
例如，我们可能使用树中的一个公钥来接收付款，这将使它在[区块链](/docs/technical/blockchain.md)上可见。如果我们不使用链码，任何人都可以获取此公钥并派生它的所有子密钥：
...
你无法看出树中的任何两个公钥（或[地址](/docs/technical/keys/address.md)）是否属于同一个钱包（即从同一个主扩展密钥派生而来）。
...
注意：1地址 ([P2PKH](/docs/technical/script/p2pkh.md))
...
注意：bc1地址 ([P2WPKH](/docs/technical/script/p2wpkh.md))
...
然后，我们可以通过对其进行 [base58check](/docs/technical/keys/base58.md#base58check) 编码来使其成为地址：

这现在是我们的扩展私钥更有用的格式，因为它更容易在计算机之间共享并导入到钱包中。

详情请参阅[扩展密钥地址](/docs/technical/keys/hd-wallets/extended-keys.md#address)。
...
**分层确定性钱包**提供了一种生成新[私钥](/docs/technical/keys/private-key.md)和[公钥](/docs/technical/keys/public-key.md)的有用方法。
...
* [助记词](/docs/technical/keys/hd-wallets/mnemonic-seed.md) – 为你的 HD Wallets 生成对用户友好的 seed。
* [扩展密钥](/docs/technical/keys/hd-wallets/extended-keys.md) – 创建主扩展密钥，并从中派生子密钥。
* [派生路径](/docs/technical/keys/hd-wallets/derivation-paths.md) – 钱包用于组织密钥的常见层级结构。
