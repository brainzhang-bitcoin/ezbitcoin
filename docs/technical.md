...
* **获取原始比特币数据**。通过在本地计算机上运行全节点，您可以快速轻松地访问原始的[区块](/docs/technical/block.md)和[交易](/docs/technical/transaction.md)数据，而无需依赖第三方 API：  
...
* **实用的命令行工具**。Bitcoin Core 程序打包了一系列命令行工具，我经常用它们进行调试 and decoding. Here are a couple of useful examples:  
  ...
  ```
* **发送原始交易**。如果您的工作是构建自己的原始[交易](/docs/technical/transaction.md)，您可以通过自己的本地节点将它们发送到比特币网络中：  
...
1. **生成您自己的[密钥](/docs/technical/keys.md)**。这是完美的起点。尝试生成您自己的[private key](/docs/technical/keys/private-key.md)、[public key](/docs/technical/keys/public-key.md)和[地址](/docs/technical/keys/address.md)。然后将 private key 导入钱包，看看是否能得到与您生成的地址相同的地址。  
   ...
   ```
2. **解码[交易](/docs/technical/transaction.md)**。学习如何解码原始交易将教给您很多关于比特币交易结构的知识，它们构成了[blockchain](/docs/technical/blockchain.md)内99%的数据。  
   ...
   * `0 vbytes`
   ...
   0 秒
3. **创建您自己的交易**。在解码交易之后，您就可以开始创建自己的交易了。这是一个大得多的里程碑（所以慢慢来），但这是自然的下一步。对其进行[签名](/docs/technical/keys/signature.md)将是棘手的部分，但如果您能成功将自己的比特币交易发送到[网络](/docs/technical/networking.md)中，那么您就可以确认自己是一个相当不错的比特币程序员了。  
...
* **哈希函数** - 能够快速且轻松地获取一些数据的 [HASH256](/docs/technical/cryptography/hash-function.md#hash256) 或 [HASH160](/docs/technical/cryptography/hash-function.md#hash160) 非常方便，因为它们在比特币中随处可见。  
  ...
  0 秒
* **反转字节顺序** - 这对我来说极其宝贵。您经常需要反转 [txid](/docs/technical/transaction/input/txid.md) 和[block hashes](/docs/technical/block/hash.md)的[byte order](/docs/technical/general/byte-order.md)，因为在原始交易数据和区块数据中使用的字节顺序与在[blockchain explorer](/explorer/)中搜索它们时的字节顺序正好相反。此外，原始比特币数据中的大多数字段都是“[little-endian](/docs/technical/general/little-endian.md)”，因此在将十六进制和十进制数相互转换时，您经常需要反转字节顺序。  
  ...
  0 秒
* **数字转换器** - 我记不清有多少次我需要将[hexadecimal](/docs/technical/general/hexadecimal.md)转换成十进制（反之亦然）。如果您愿意，可以使用在线工具，但没有什么比打开终端并使用自己编写的脚本转换数字更好的了。  
...
**要负责任**。如果您在创建一个处理 [private keys](/docs/technical/keys/private-key.md) 或为其他人创建[transactions](/docs/technical/transaction.md)的工具（例如钱包），您需要非常小心。自己犯错导致丢失自己的币是一回事，但犯错导致他人丢失币则是另一回事，所以千万不要掉以轻心。
...
比特币是去中心化的开源软件。如果您愿意，您可以生成自己的[keys](/docs/technical/keys.md)并构建自己的[transactions](/docs/technical/transaction.md)，没有人可以阻止您。这也是比特币之所以成为比特币的一部分。
