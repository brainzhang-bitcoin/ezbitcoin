...
Once you've got a wallet, you've got everything you need to start receiving and [sending](/beginners/sending/) bitcoins.
...
A hardware wallet is the best option for [secure storage](/beginners/security/) of your bitcoins.
...
A bitcoin wallet manages all the [keys](/beginners/guide/keys-addresses/) you need to access your bitcoin, and some of these need to be kept private. So the last thing you want to do is download your wallet from an unreliable source. If you download directly from the wallet's website, you can be confident that you're getting an authentic version of the software.
...
During the initial setup of your wallet you will be given a 12 or 24 word [seed](/technical/keys/hd-wallets/mnemonic-seed/).
...
This seed is the source of all your [keys and addresses](/beginners/guide/keys-addresses/). Therefore, it allows you to recover all of your bitcoins if you end up losing your wallet for whatever reason.
...
You use one of your [addresses](/technical/keys/address/) when you want to *receive* a payment. The [way Bitcoin works](/beginners/how-does-bitcoin-work/) is a certain amount of bitcoin will get *locked* to your address in a [transaction](/beginners/guide/transactions/).
...
When you *send* bitcoins to someone else, your wallet will create a new transaction that *unlocks* an amount of bitcoin you've received using the corresponding [private key](/technical/keys/private-key/), and locks up an amount of bitcoin to someone else's address.
...
1. Manages the [addresses](/technical/keys/address/) required for receiving bitcoins.
2. Manages the corresponding [private keys](/technical/keys/private-key/) for sending bitcoins.
3. Creates [transactions](/technical/transaction/) and sends them into the network.
...
A hardware wallet stores all of your [private keys](/beginners/guide/private-keys/) on the device. So when you connect the hardware wallet to a computer to make a [transaction](/beginners/guide/transactions/), all of the [signing](/beginners/guide/digital-signatures/) takes place on the device and your private keys are never exposed to the Internet.
...
A hardware wallet is the first step you should take to improve the [security of your bitcoins](/beginners/security/).
...
If anything, it's the *ultimate* option for using Bitcoin, because it allows you to verify every single payment you receive **without having to trust** any other [node](/technical/networking/node/) on the network (because you are one).

The downside is that you have to download the entire [blockchain](/technical/blockchain/) before you can get started, which slows down the initial setup process. But if you've got the computer hardware and time to set it up, it's a perfectly good option for a bitcoin wallet.
...
If you keep your bitcoins on an [exchange](/beginners/exchanges/), they have control over your bitcoins instead. They can determine how much you can withdraw, and where you can send your bitcoins. Worse still, if the exchange disappears, then so do your bitcoins.
...
Once you've got your bitcoin wallet set up, you're all set to [buy your first bitcoins](/beginners/exchanges/).